...
sequenceDiagram
    ...
    A->>M: createImplementationTasks / parallel createTaskOperations
    A->>R: Hermes run / agent run / 写源码、更新任务、创建 build job
    U->>UI: 构建
    UI->>B: build(job)
...
7. `AgentService` 创建 `hermes_execution_runs`，记录模式、并发上限和 base source hash。
8. `HermesParallelScheduler.nextBatch()` 根据任务契约、文件锁、高风险路径和并发设置选择下一批任务。
9. 批次任务标记为 `running`；每个任务创建 `hermes_agent_runs`，记录 batch/agent index、锁定路径和 scratch work dir。
10. `HermesAgentWorker` 把 canonical source 复制到独立 scratch source。
11. 每个逻辑子 Agent 调用 `OpenAiClient.createTaskOperations()` 生成文件操作 JSON，并在 scratch source 内跑解析、契约守护、确定性预检、Hermes review 和本地 guard。
12. 所有 worker 返回后，`HermesMergeCoordinator` 检查 touched paths、unsafe path、wildcard 和任务契约，只有无冲突结果会写回 canonical source。
13. 合并成功的任务标记 `done`，`hermes_agent_runs` 标记 `done` 并记录 merged source hash；合并冲突或 worker 失败会把相关任务标记 `failed`。
14. 如果还有任务，计划状态回到 `planned`；如果没有任务，状态为 `generated`。
15. 打包当前源码为 `project.zip`，写入 `build.log`。
16. `build_jobs` 状态更新为 `generated/ready_for_build`；若批次任务契约要求构建校验，阶段会标记为 `ready_for_build_after_task`。

并行边界：

- 并行子 Agent 是 App 内逻辑 worker，不是外部进程。
- 设置页可选择串行、2 个、3 个；默认由 `BuildBackendSettings.parallelAgentLimit()` 控制。
- 同一批次内不能有文件锁冲突；Gradle、Manifest、共享资源、需要立即构建的高风险任务会串行。
- API 429/rate limit 会给出降级提示，建议改成串行或 2 个后重试。
...
4. `HermesRepairShardingPolicy` 尝试把低耦合诊断拆成修复分片：例如不同文件上的资源缺失、独立 Java `cannot find symbol`。
5. 如果分片互不冲突且设置页并发上限大于 1，修复会走并行 scratch source；合并成功后写回 canonical source。
6. 如果构建日志包含 Gradle 依赖、Manifest merge、构造/方法签名不匹配等共享风险，或并行执行失败但尚未成功合并，回落到单 Agent 修复。
7. 如果并行合并已经写入部分无冲突修复但仍有冲突，流程不会再自动覆盖这些改动；用户可再次点击 Repair 继续处理剩余失败。
8. 修复成功后产出新的 generated job，时间线刷新；**不自动重新构建**，由用户再次点击 Build。
9. 修复失败则 job 进入 `repair_failed`，用户可再次手动修复或改 Build。
10. `build_jobs.retry_count` 记录已尝试次数，但不再作为自动修复上限。
...
- 任务列表默认折叠，由 `ProjectTaskListDisplayPolicy.visibleTasks()` 只展示运行中、失败、等待合并和第一个待执行任务；展开后按任务契约分组显示完整列表。
- 展开任务时，`HermesAgentRunDisplayPolicy` 会显示批次、Agent 编号、锁定路径和运行/等待合并/失败状态。
