...
final class RuntimeCrashPlaybook {
    ...
    private static final Clause[] CLAUSES = {
            ...
            new Clause(new String[]{"ClassCastException"},
                    "- ClassCastException (often at findViewById): the Java cast does not match the widget type declared for that id in the layout XML (e.g. casting to (RadioButton) an id that is a TextView/MaterialTextView). Fix the MISMATCH — either change the cast to the layout's actual widget type, OR change the layout widget at that id to match the cast. Pick by the methods the code calls: isChecked()/setChecked()/setOnCheckedChangeListener need a CompoundButton (RadioButton/CheckBox/Switch); setText() only needs a TextView.",
                    "- ClassCastException（常见于 findViewById）：Java 的强转类型和布局 XML 里该 id 声明的控件类型对不上（例如把一个本是 TextView/MaterialTextView 的 id 强转成 RadioButton）。修这处不一致——要么把强转改成布局里实际的控件类型，要么把该 id 对应的布局控件改成强转的类型。按代码调用的方法判断意图：调用 isChecked()/setChecked()/setOnCheckedChangeListener 的需要 CompoundButton（RadioButton/CheckBox/Switch），只调 setText() 的用 TextView 即可。"),
