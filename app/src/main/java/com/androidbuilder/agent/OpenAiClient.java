...
public class OpenAiClient {
    ...
    private static String milestoneTasksSystemPromptText(boolean chinese) {
        ...
        return "You split ONE milestone of an Android app into a short sequential implementation task list. " +
                ...
                "Use 1 to 6 tasks — prefer FINER, single-deliverable tasks over one broad task (a broad task makes a weak model under-deliver: it pads the patch with unrelated files and silently drops the real deliverable). " +
                "Each task must produce ONE cohesive deliverable named in its title (for example 'DatabaseHelper and all tables', 'Category model and CategoryDao', 'home screen layout and its drawables'), and its instruction must list the exact files it creates. " +
                "NEVER bundle Java class creation (DatabaseHelper, models, DAOs, Activities, Adapters) together with drawable or layout XML in the same task: Java classes get their own task(s), drawables/layouts get their own task(s), values/themes/menu get their own task(s), and Gradle configuration stays separate from Java source wiring. A task whose title names a Java class must create that .java file. " +
    ...
    private static String taskOperationsSystemPromptText(boolean chinese, String dependencyPolicyPrompt) {
        ...
        return "You execute one small Android coding task by returning file operations in a FENCED text format (NOT JSON). " +
                ...
                "Follow Google's Material Design for every screen, using the toolkit that is actually available: define a consistent color palette in colors.xml + a Material/AppCompat theme in themes.xml (colorPrimary, colorPrimaryVariant/Dark, colorAccent or colorSecondary, a surface/background, and a text-on-surface color) and reuse those colors everywhere — never hard-code random hex per screen; use an 8dp spacing grid (8/16/24dp margins and padding); lay screens out as cards with rounded corners and light elevation (MaterialCardView when the Material Components dependency is declared, otherwise androidx CardView or a rounded @drawable/bg_card with android:elevation); give the screen a top app bar/Toolbar title, a FloatingActionButton or clear primary button for the main action, and BottomNavigationView/tabs for primary navigation when there are multiple sections; make touch targets at least 48dp; and show a friendly empty-state (icon + text) instead of a blank list. Keep colors, corner radius, and spacing consistent across ALL screens so the app looks like one coherent Material app, not unstyled white screens. " +
