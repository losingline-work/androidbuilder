    ...
    private String repairInstruction(String buildLog, boolean chinese, boolean escalate) {
        ...
        if (chinese) {
            return escalationClause
                    ...
                    + "依赖无法解析/下载时（依赖策略限制、Could not find/download <group:artifact>、网络不可达）：首选【保留功能】——删除该第三方库，并用 Android SDK/Java/XML 重新实现同样的功能，让 app 继续可用：图表→用 Canvas 自绘 View；图片加载→BitmapFactory（+ HttpURLConnection 后台线程）；动画→属性动画/View 动画；JSON→org.json；网络请求→HttpURLConnection；二维码→自绘或系统相机。也可改用已验证依赖（" + DependencyCatalog.coordinatesSummary() + "）。要灵活：不要直接删掉功能或留空白页；只有当用 SDK 确实无法实现时，才保留最小占位并在 summary 里说明原因。"
        ...
        return escalationClause
                ...
                + "When a dependency cannot be resolved or downloaded (dependency policy, 'Could not find/download <group:artifact>', or an unreachable network): PREFER KEEPING THE FEATURE — remove the third-party library and reimplement the same feature with Android SDK/Java/XML so the app keeps working: charts -> a custom Canvas-drawn View; image loading -> BitmapFactory (with HttpURLConnection on a background thread); animation -> property/View animation; JSON -> org.json; HTTP -> HttpURLConnection; QR codes -> a custom view or the camera. You may also switch to a verified catalog library (" + DependencyCatalog.coordinatesSummary() + "). Be flexible: do NOT just delete the feature or leave a blank screen; only leave a minimal placeholder when an SDK implementation is genuinely impossible, and say so in the summary. "
